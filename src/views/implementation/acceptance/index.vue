<template>
  <div class="acceptance-container">
    <div class="page-header">
      <h1 class="page-title">项目验收管理</h1>
      <div class="page-desc">管理项目验收相关材料、流程和会议，确保项目顺利结题归档</div>
    </div>
    ...
    <a-tabs 
      ...
      <a-tab-pane key="application-submit" class="applicant-role">
        <template #tab>
          <CheckCircleOutlined class="applicant-role-icon" />
          提交验收
        </template>
...
import { UserOutlined, CheckCircleOutlined, SettingOutlined, TeamOutlined } from '@ant-design/icons-vue'

// 导入原来的子菜单页面组件
const ApplicantManageComponent = defineAsyncComponent(() => import('@/views/acceptance/applicant/manage.vue'))
const ApplicantSubmitComponent = defineAsyncComponent(() => import('@/views/acceptance/applicant/submit.vue'))
const ManagementCenterComponent = defineAsyncComponent(() => import('@/views/acceptance/management/center.vue'))
const ManagementConclusionComponent = defineAsyncComponent(() => import('@/views/acceptance/management/conclusion.vue'))
