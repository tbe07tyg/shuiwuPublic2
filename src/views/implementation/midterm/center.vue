...
-->
<template>
  <div class="midterm-management-center">
    <!-- 页面头部 -->
    ...
    <div class="content-card flow-description">
      ...
      <div v-show="showFlowDescription" class="flow-content">
        <a-steps :current="3" size="small" direction="horizontal" status="finish">
          <a-step title="申请提交" description="申报单位提交中期检查申请" status="finish" />
      ...
      <div v-if="currentReview" class="review-modal">
        ...
        <a-descriptions :column="2" bordered size="small">
          ...
          <a-descriptions-item label="预期中期时间">
            {{ currentReview.expectedMidtermDate }}
          </a-descriptions-item>
        </a-descriptions>
    ...
    >
      <div class="batch-review">
        <p>已选择 {{ selectedRowKeys.length }} 个中期申请进行批量审核</p>
...
const applicationList = ref([
  {
    ...
    submitTime: '2024-06-15 10:30',
    expectedMidtermDate: '2024-07-10',
    materials: [
      { id: 1, name: '中期报告', type: 'required', status: 'pending', size: '3.2MB' },
      { id: 2, name: '进展总结', type: 'required', status: 'pending', size: '2.1MB' },
      { id: 3, name: '阶段成果', type: 'required', status: 'pending', size: '4.5MB' }
    ]
  },
  {
    id: 2,
    ...
    submitTime: '2024-06-18 16:45',
    expectedMidtermDate: '2024-07-15',
    materials: [
      { id: 1, name: '中期报告', type: 'required', status: 'approved', size: '2.8MB' },
      { id: 2, name: '进展总结', type: 'required', status: 'approved', size: '1.9MB' }
    ]
  }
])
...
const getProgressStatus = (status) => {
  if (status === 'midterm_failed') return 'exception'
  if (status === 'midterm_passed' || status === 'midterm_conditional') return 'success'
  return 'active'
}
...
const getProgressText = (status) => {
  const textMap = {
    ...
    'midterm_passed': '中期检查流程完成',
    'midterm_conditional': '有条件通过中期',
    'midterm_failed': '中期检查未通过'
  }
...
const handleScheduleMeeting = (record) => {
  router.push(`/implementation/midterm/meeting?projectId=${record.id}&action=schedule`)
}

const handleInputConclusion = (record) => {
  router.push(`/implementation/midterm/meeting?projectId=${record.id}&action=conclusion`)
}
...
const goToMeetingManagement = () => {
  router.push('/implementation/midterm/meeting')
}
