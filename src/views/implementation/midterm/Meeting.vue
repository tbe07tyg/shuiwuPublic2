<template>
  <div class="midterm-meeting-page">
    <!-- 页面头部 -->
    <div class="page-header">
      <div class="header-content">
        <h1 class="page-title">
          <ScheduleOutlined /> 项目中期检查会议管理
        </h1>
        <p class="page-desc">组织和管理项目中期检查会议，收集专家意见，生成中期检查结论</p>
      </div>
    ...
    <!-- 中期检查会议计划与组织区 -->
    <a-card class="plan-section" title="中期检查会议计划">
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="8">
            <a-form-item label="会议主题" required>
              <a-input v-model:value="meetingPlan.title" placeholder="项目中期检查评审会" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-form-item label="会议议程安排">
          <a-textarea v-model:value="meetingPlan.agenda" :rows="2" placeholder="项目进展汇报、阶段成果展示、专家评审、问题讨论等" />
        </a-form-item>
      </a-form>
    </a-card>
        ...
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.key === 'role'">
            <a-select v-model:value="record.role" style="width: 120px">
              <a-select-option value="评审专家">评审专家</a-select-option>
    ...
    <a-card class="opinion-section" title="专家意见与会议纪要">
      <a-table :columns="opinionColumns" :data-source="opinions" rowKey="id" bordered>
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.key === 'opinion'">
            <a-textarea v-model:value="record.opinion" :rows="2" placeholder="中期检查评审意见" />
          </template>
      ...
      <a-form-item label="中期检查会议纪要" style="margin-top: 16px;">
        <a-textarea v-model:value="meetingSummary" :rows="3" placeholder="请输入中期检查会议纪要..." />
      </a-form-item>
    </a-card>

    <!-- 中期检查结论文件上传区 -->
    <a-card class="conclusion-section" title="中期检查结论文件上传">
      <a-upload
        ...
        :max-count="1"
      >
        <a-button type="link">上传中期检查结论文件</a-button>
      </a-upload>
      <!-- 若结论模板需从外部系统获取，显著标记 -->
      <a-tag color="orange">中期检查结论模板如需外部系统获取请在此标记</a-tag>
    </a-card>
  </div>
</template>
...
const meetingPlan = ref({
  ...
  location: '',
  title: '项目中期检查评审会',
  agenda: '项目进展汇报、阶段成果展示、专家评审、问题讨论等'
})
...
const materials = ref([
  { name: '中期检查会议通知', fileList: [], fromExternal: false },
  { name: '中期检查会议议程', fileList: [], fromExternal: false },
  { name: '评审专家名单', fileList: [], fromExternal: false },
  { name: '项目中期检查报告', fileList: [], fromExternal: false },
  { name: '阶段性成果材料', fileList: [], fromExternal: false },
...
const opinionColumns = [
  ...
  { title: '中期检查评审意见', key: 'opinion', width: '50%' },
...
function handleSendMeetingNotice() {
  // TODO: 对接OA系统会议通知/日程
  message.success('中期检查会议通知已发送（模拟）')
}
