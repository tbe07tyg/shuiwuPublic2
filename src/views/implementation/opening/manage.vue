...
-->
<template>
  <div class="opening-application-manage">
    <!-- 页面头部 -->
    ...
    <div class="content-card flow-description">
      ...
      <div v-show="showFlowDescription" class="flow-content">
        <a-steps :current="3" size="small" direction="horizontal" status="finish">
          <a-step title="提交申请" description="准备开题报告等材料并提交申请" status="finish" />
...
const applications = ref([
  ...
  {
    id: 3,
    projectName: '智慧农业灌溉系统',
    projectCode: 'HS2024003',
...
const goToSubmit = () => {
  router.push('/opening/application/submit')
}
...
const getProgressText = (status, record) => {
  const textMap = {
    'material_reviewing': '正在审核开题材料',
    'meeting_preparing': '材料审核通过，待安排会议',
    'meeting_scheduled': `开题会议已安排：${record.meetingDate}`,
    'opening_passed': '开题已通过',
...
const handleViewDetail = (record) => {
  router.push(`/opening/application/detail/${record.id}`)
}

const handleEdit = (record) => {
  router.push(`/opening/application/edit/${record.id}`)
}
